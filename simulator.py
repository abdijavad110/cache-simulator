...
if __name__ == "__main__":
    ...
    parse_trd.setDaemon(True)
    stg_trd = threading.Thread(
        target=poll_q,
        # daemon=True
    )
    stg_trd.setDaemon(True)
    parse_trd.start()
    stg_trd.start()
    # monitoring:
    print("\n\n\n\n")
    ...
    try:
        while parse_trd.is_alive() or len(req_Q) != 0:
            ...
            print "requests len:\t", len(req_Q), " WCQ len:", len(
                manager.cache.WCQ), "/", manager.cache.WCQ_max_size, " SPQ len:", len(
                manager.cache.SPQ), " QT: %.1f" % manager.cache.qt
            ...
            print "cache:: hits:", hits, " ram hits:", ram_hits, " misses:", misses, " writes:", writes, ' hit ratio: %.4f' % (
                    (hits + ram_hits) / ttt * 100), ' WE: %.4f' % (
                    hits / tt), " RE:", manager.cache.ram_write_evict_cnt, ' SE:', manager.cache.ssd_write_evict_cnt
